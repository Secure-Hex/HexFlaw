...
@pytest.fixture
def custom_home(monkeypatch, tmp_path: Path) -> Path:
    monkeypatch.setattr(language_service, "global_home", lambda: tmp_path)
    return tmp_path


def test_edit_seeds_builtin_and_writes_custom(custom_home: Path, monkeypatch) -> None:
    ...
    svc = LanguageService()
    assert "my_custom_sink(" in svc.get("python").sink_patterns


def test_edit_no_changes_keeps_nothing(custom_home: Path, monkeypatch) -> None:
    monkeypatch.setattr(lang_cmd, "_open_in_editor", lambda text: None)
    lang_cmd.edit_language("python")
    assert not (custom_home / "languages" / "custom" / "python.json").exists()


def test_edit_invalid_json_rejected(custom_home: Path, monkeypatch) -> None:
